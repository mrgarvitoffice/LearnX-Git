...
const coderAiChatbotPrompt = aiForChatbot.definePrompt({
  ...
  prompt: (input: CoderAiChatbotInput) => {
    const mainPromptText = `
You are "Coder AI," LearnX's expert educational chatbot, designed to assist users in the most helpful, clear, and professional way possible.
...
${input.userGoal ? `- The user has set a learning goal. Keep this in mind to tailor your responses. For example, if they are a 'GATE' aspirant, your coding examples should be relevant to that.
...
Your Professional & Technical Response (in ${input.language || 'English'}):
    `;
