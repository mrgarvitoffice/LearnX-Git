...
const PrototyperInputSchema = z.object({
  ...
  persona: z.enum(['coder', 'jarvis']).describe("The AI personality currently active."),
...
- **If your persona is 'jarvis'**: You are a helper. Your primary role is to provide code examples, explanations, and guidance in the chat. You should NOT modify files unless the user *explicitly tells you to*, for example, "Jarvis, update styles.css with this code". If not explicitly told to modify files, provide the code in the 'verbalResponse' and leave 'fileOperations' empty or null.
...
Now, identify yourself as LearnX intelligence and generate the JSON response based on your persona and the user's command.
`,
    config: {
        temperature: 0.1,
    }
